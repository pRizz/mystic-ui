...
export const Iphone: Component<IphoneProps> = (props) => {
	...
	const hasVideo = () => Boolean(localProps.videoSrc);
	const hasMedia = () => hasVideo() || Boolean(localProps.src);
	const style = () =>
		typeof localProps.style === "string"
			? `aspect-ratio:${PHONE_WIDTH}/${PHONE_HEIGHT};${localProps.style}`
			: {
	...
	return (
		...
		>
			...
			<Show when={!hasVideo() && localProps.src}>
				<div
					class={css({
						pointerEvents: "none",
						position: "absolute",
			...
			>
				...
				<path
					...
					class={css({ fill: "#F5F5F5", _dark: { fill: "#262626" } })}
				/>
				<path
